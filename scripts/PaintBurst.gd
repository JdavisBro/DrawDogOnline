...
func _ready():
	...
	queue.append(Vector2i(position.round()))
...
func _process(delta: float):
	...
	#var end_time := Time.get_ticks_msec() + 1 # this is in the base game. time cap of 2ms
		...
		#end_time += 2
	var end_time: float = Time.get_ticks_msec() + (3 - min(10, len(get_tree().get_nodes_in_group("paintbursts"))) / 5)
	while queue and checked < CHECKED_CAP and Time.get_ticks_msec() < end_time:
		var pos: Vector2i = queue.pop_front()
		if color_collide > -1 and paint.paint.at(pos.x, pos.y) != color_collide:
			continue
		...
		add_newpos(Vector2i(pos.x + 1, pos.y))
		add_newpos(Vector2i(pos.x, pos.y + 1))
		add_newpos(Vector2i(pos.x - 1, pos.y))
		add_newpos(Vector2i(pos.x, pos.y - 1))
		...
		if cur_dist < farthest_dist and (color_collide == -1 or Settings.fill_bucket_do_corners):
			...
			add_newpos(Vector2i(pos.x + 1, pos.y + 1))
			add_newpos(Vector2i(pos.x - 1, pos.y + 1))
			add_newpos(Vector2i(pos.x + 1, pos.y - 1))
			add_newpos(Vector2i(pos.x - 1, pos.y - 1))
...
func add_newpos(newpos: Vector2i):
